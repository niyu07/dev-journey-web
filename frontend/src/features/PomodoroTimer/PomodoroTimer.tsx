...
export const PomodoroTimer = () => {
  ...
  useEffect(() => {
    if (timeRemaining < 0) { // Changed from === 0 to < 0 to prevent issues
      setIsActive(false);
  ...
  const handleTimeChange = (newMode: Mode, newTime: number) => {
    if (newTime > 0 && newTime <= 180) { // Limit time to a reasonable range
      const newModes = { ...modes, [newMode]: { ...modes[newMode], time: newTime } };
      setModes(newModes);
    ...
    <div>
        ...
        {Object.keys(modes).map(key => {
          ...
          return (
            <div key={key} style={{ margin: '5px' }}>
              <button onClick={() => switchMode(modeKey)} disabled={mode === modeKey}>
                {modes[modeKey].name}
              </button>
              <input 
                type="number"
                value={modes[modeKey].time}
                onChange={(e) => handleTimeChange(modeKey, parseInt(e.target.value, 10))}
      ...
      <div style={{ margin: '10px 0' }}>
        {presetTimes.map(time => (
          <button key={time} onClick={() => handleSetPresetTime(time)} style={{ marginRight: '5px' }}>
            {time}分
          </button>
        ))}
      </div>
      <div>
        <button onClick={handleStartPause}>{isActive ? '一時停止' : 'スタート'}</button>
