...
const classificationOptions = ["個人", "個人事業主"];
...
const Accounting: React.FC = () => {
  ...
  const fetchCategories = React.useCallback(async () => {
    try {
      const response = await fetch(
        "http://localhost:8000/api/accounting/categories",
      );
      if (!response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingId) {
      // Update logic
      const response = await fetch(
        `http://localhost:8000/api/accounting/${editingId}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      // Create logic
      if (
        classification === "個人事業主" &&
        entryType === "経費" &&
        !receiptProcessed
      ) {
        setError(
          "個人事業主の経費を登録するには、レシートのアップロードが必要です。",
        );
        return;
      }
      const response = await fetch("http://localhost:8000/api/accounting", {
  ...
  const handleReceiptUpload = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:8000/api/accounting/receipt",
        {
          method: "POST",
          body: formData,
        },
  ...
  const isFormValid =
    date && amount && category && description && classification;
